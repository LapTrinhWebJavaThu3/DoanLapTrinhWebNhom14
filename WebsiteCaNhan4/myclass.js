...
//Xóa mục tiêu
    ...
    $('.btnThaoLuan').click(function(){
        ...
        if(title_discussion_course==""){
            ...
            if(content_discusstion_course.length<13 || content_discusstion_course.length>1550){
                alert("Nội dung bình luận từ 6 đến 1500 ký tự");
                This.find("i").remove();
                This.removeClass("warning");
            }else{
                $('.chemanhinh').show();
    ...
    $('body').delegate('.btn_comment_child','click',function(){
        ...
        var user_id = $('.user_id').val();
	var course_id_main=$('.course_id').val();
        var warning='<i class="icon-sun-stroke animated spin"></i>';
    ...
    $('body').delegate('.close_edit_content','click',function(){
        ...
        This.closest(".item-discustion").find(".SuaThongBaoGiangVien").fadeIn();
    ...
    $('body').delegate('#thaoluanunit','click',function(){
        ...
        if(!$(this).hasClass('contenteditor')){
            ...
            This.append('<span class="thembinhtheounit btn btn-success" style="  top: 200px;float: right;position: relative;">Gửi</span><span class="insert-my-media btn btn-success" data-id="'+unit_id+'">Thêm hình ảnh</span><div class="append-content-discussion"></div>');
...
//    Click vào cái chuông thì ẩn hiện nội dung thông báo và cập nhật lại trạng thái của comment thành comment củ
    ...
    // Cách này do mình edit lại để phù hợp với giao diện website
   ...
   $('body').delegate('.courseitem','mouseenter',function(){
	...
	var orderby = $('.sapxepkhoahoctrangchu option:selected').val();
...
//    ẩn scroll màn hình ngoài khi bấm vào khóa học hiện tại sau đó bấm vào start course
	...
	$('.hasmycredpoints-dongyhockhoahoc').click(function(event){
    	    event.stopPropagation();
     	    event.preventDefault();
      	    if($(this).hasClass('hasmycredpoints')){
            $(".chemanhinh").css("display","block");
            $("#ajaxloader").show();
            $.ajax({
                type: "POST",
...
//		                    }
                ...
                // return $("#vaotranghockhoahoc").submit();
    ...
    $('body').delegate('.sapxepkhoahoctrangchu','change',function(){

        $('.sotrang').trigger('click');
    });


   $('body').delegate('.comment-content-user img','click',function(){

        $('body').css('overflow', 'hidden');

        var bigimg = '<div class="bigimg">' +
            '<span class="close-big-img"><i class="icon-x" /></span><img src="'+$(this).attr("src")+'"/></div>';

        $('body').append(bigimg);

    });

    $('body').delegate('.close-big-img','click',function(){
        $('.bigimg').remove();
        $('body').css('overflow-y', 'scroll');
    });

    $('body').delegate('.bigimg','click',function(){
        $('.close-big-img').trigger('click');
    });

    $('body').delegate('.comment-content-user img','mouseenter',function() {
        $(this).parent().addClass('img-zoom-parent');
        $(this).parent().css('max-width',$(this).attr('width'));
        if(!$(this).parent().is('p')){
            $(this).parent().css('padding','10px 0px');
        }
    });

    $('body').delegate('.comment-content-user img','mouseleave',function() {
        $(this).parent().removeClass('img-zoom-parent');
    });

  $('.ThongBao').click(function(){
        $('.discussion-content').css("display","none");
        $('.thongbaogiangvien').css("display","block");
        $('.tab_tab_settings li').removeClass('activeli');
       ...
       $.ajax({
            ...
            url: ajaxurl,
            data: {
                action: 'CapNhatTrangThaiThongBaoHocVien'
            },
            cache: false,
            success: function (result) {
                $('.sothongbao_giangvien').css('display','none !important');
            }
        });

    });

    //Thêm nội dung thông báo của giảng viên cho học viên
    $('body').delegate('.btnThongBaoGiangVien','click',function(){
        var This = $(this);
        var course_id=$('.course_id').val();
        var title_discussion_course=$('.title-thongbaogiangvien').val();
        var content_discusstion_course=get_tinymce_content('txtThongBao');
        var user_id = $('.user_id').val();
        ...
        if(title_discussion_course==""){
            ...
            This.removeClass("warning");
                ...
                $.ajax({
                    type: "POST",
                    url: ajaxurl,
                    data: { action: 'ThemThongBaoCuaGiangVien',
                        security: $('#security').val(),
                        cdtitle: title_discussion_course,
                        cdcontent: content_discusstion_course,
                        cdcourseid:course_id,
                    ...
                    success: function (result) {
                        ...
                        /*append thảo luận*/
                        var contentdc= $(result).filter('.result').html();
                        $(".NoiDungDuocThem").prepend(contentdc);
                        $(".contentdcs").fadeOut(200);
                        tinyMCE.activeEditor.setContent('');
                    }
                });
    ...
    $('.discussion').click(function(){
        $('.discussion-content').css("display","block");
        $('.thongbaogiangvien').css("display","none");
        $('.tab_tab_settings li').removeClass('activeli');
        $(this).addClass('activeli');
    ...
    //Xóa thông báo giảng viên
    $('body').delegate('.XoaThongBaoGiangVien','click',function(){
        ...
        $.ajax({
            type: "POST",
            url: ajaxurl,
            data: { action: 'XoaThongBaoGiangVien',
                //security: $('#security').val(),
                id: cmt_id
            },
            cache: false,
   ...
   function EditorThongBao(vitri,id_editor,noidung,title){
        iconclose = "<div class='close_edit_content' style='text-align: right'><i class='icon-close-off-2' style='position: relative;margin: 0;cursor: pointer;color: #999999'></i> </div> ";
        title = iconclose+'<input type="text" class="title form-control" value="'+title+'" placeholder="Nhập vào tiêu đề thảo luận" >';
        button = '<div style="float: right;width: auto;" class="btn btn-success btn_capnhathongbao" >Lưu</div>';
        buttonAddMedia = '<span class="insert-my-media btn btn-success" data-id="'+id_editor+'">Thêm hình ảnh</span>';
        htmlSource = title+'<textarea id="editor-content'+id_editor+'" class="wp-editor-area editor-content" >'+noidung+'</textarea>'+button+buttonAddMedia;
    ...
    //Sửa danh sách giảng viên
    $('body').delegate('.SuaThongBaoGiangVien','click',function(){
        ...
        $(this).closest(".item-discustion").find(".NoiDungCMTUser").fadeOut();
        //tinymce.remove();
        EditorThongBao(div_editor,id_cmt,content,title);
        $(this).removeClass('warning');
    });

    $('body').delegate('.btn_capnhathongbao','click',function(){
        ...
        $.ajax({
            type: "POST",
            url: ajaxurl,
            data: { action: 'CapNhatThongBaoGiangVien',
                cmt_id: cmt_id,
                title:title,
                content:content
            },
...
//trả lời đánh giá
    $('body').delegate('.rely_review','click',function(){
        $(this).css('display','none');
        $(this).parent().find('.hidden_rely_review').css('display','block');
        $(this).parent().parent().find('.child-review').css('display','block');
    });

    $('body').delegate('.hidden_rely_review','click',function(){
        $(this).css('display','none');
        $(this).parent().find('.rely_review').css('display','block');
        $(this).parent().parent().find('.child-review').css('display','none');
    });


    $('body').delegate('.traloidanhgia','click',function(){
        var tieude = $(this).parent().find('.tieudetraloidanhgia').val();
        var noidung = $(this).parent().find('.noidungtraloidanhgia').val();
        var id = $(this).attr('data-course');
        var iddanhgia = $(this).attr('data-id');


        if(noidung.length<6){
            alert("Nội dung trả lời ít nhất là 6 ký tự");
        }else{
            $('.chemanhinh').css('display','block');
            $('#ajaxloader').removeClass('disabled');
            $(this).parent().find('.loadingdanhgiacon').removeClass('anpopupthongtinkhoahoc');
            $.ajax({
                type: "POST",
                url: ajaxurl,
                data: {
                    action: "traloidanhgia",
                    tieude: tieude,
                    noidung: noidung,
                    iddanhgia : iddanhgia,
                    course_id : id
                ...
                success: function (result) {
                    ...
                    $('#ajaxloader').addClass('disabled');
                    if ($.isNumeric(result)) {
                        $(this).parent().find('.loadingdanhgiacon').addClass('anpopupthongtinkhoahoc');
                        $('.icon-x.danhgia').trigger('click');
                    } else {
                        alert(result);
                    }
    ...
    /*Hiệu chỉnh trả lời đánh giá (Xóa sửa)*/
    $('body').delegate('.child-review .comment-body','hover',function(e){
        e.stopPropagation();
        $(this).children().find('.HieuChinh-traloidanhgia').fadeIn(200);
    });
    $('body').delegate('.child-review .comment-body','mouseleave',function(){
        $(this).find('.HieuChinh-traloidanhgia').fadeOut(200);
    });


    /* Xóa trả lời đánh giá */
    $('body').delegate('.Xoatraloidanhgia','click',function(){
        var r = confirm("Bạn có muốn xóa trả lời này ?");
        if(r==false)
        {
            return;
        }
        var cmt_id = $(this).parent().find('.id-traloi-danhgia').val();
        var This = $(this);
        $.ajax({
            type: "POST",
            url: ajaxurl,
    ...
    $('body').delegate('.Suatraloidanhgia','click',function(){
        var id = $(this).parent().find('.id-traloi-danhgia').val();
        var danhgia = $(this).closest('.noidungdanhgia').find('.noidungtraloidangia');
        var tieude = danhgia.find('.tieude').text();
        var noidung = danhgia.find('.noidung').text();
        $(this).closest('.noidungdanhgia').css('display','none');
        var dulieu='<div class="close_edit_traloi_danhgia"><i class="icon-close-off-2" style="position: relative;margin: 0;cursor: pointer;color: #999999"></i> </div>';
        dulieu+='<input type="text" class="tieudetraloidanhgia" value="'+tieude+'" />';
        dulieu+='<textarea rows="4" class="noidungtraloidanhgia" placeholder="Nội dung trả lời">'+noidung+'</textarea>';
        dulieu+='<span data-id="'+id+'" class="btn btn-primary capnhaptraloidanhgia">lưu</span>';
        dulieu+='<p class="loadingdanhgiacon anpopupthongtinkhoahoc"><i class="icon-refresh glyphicon-refresh-animate"></i>Đang lưu...</p>';

        $(this).closest('.comment-body').find('.edit-traloidanhgia').append(dulieu);

    });


    $('body').delegate('.close_edit_traloi_danhgia','click',function(){
        $(this).closest('.comment-body').find('.noidungdanhgia').css('display','block');
        $(this).closest('.comment-body').find('.edit-traloidanhgia').empty();
    });

    /* Cập nhật trả lời đánh giá */
    $('body').delegate('.capnhaptraloidanhgia','click',function(){
        var id = $(this).attr('data-id');
        var tieude = $(this).parent().find('.tieudetraloidanhgia').val();
        var noidung = $(this).parent().find('.noidungtraloidanhgia').val();
        if(noidung.length<6){
